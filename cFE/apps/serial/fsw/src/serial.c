...
void SerialMain( void )
{
    ...
    */
    while (CFE_ES_RunLoop(&RunStatus) == TRUE)
    {
        ...
        status = CFE_SB_RcvMsg(&SERIALMsgPtr, SERIAL_CommandPipe, 500);
...
void SERIALInit(void)
{
    ...
    **  messages
    */
    CFE_SB_CreatePipe(&SERIAL_CommandPipe, SERIAL_PIPE_DEPTH,"SERIAL_CMD_PIPE");
    CFE_SB_Subscribe(SERIAL_CMD_MID, SERIAL_CommandPipe);
    CFE_SB_Subscribe(SERIAL_SEND_HK_MID, SERIAL_CommandPipe);
...
void SERIAL_ProcessCommandPacket(void)
{
    ...
    MsgId = CFE_SB_GetMsgId(SERIALMsgPtr);
...
void SERIAL_ProcessGroundCommand(void)
{
    ...
    CommandCode = CFE_SB_GetCmdCode(SERIALMsgPtr);

    /* Process "known" SERIAL app ground commands */
    switch (CommandCode)
    {
        case SERIAL_NOOP_CC:
